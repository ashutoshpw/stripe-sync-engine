...
export function createSubscriptionItem(
  ...
  return {
    ...
    billing_thresholds_subscription_item:
      overrides.billing_thresholds_subscription_item ?? undefined,
...
export function createSetupIntent(overrides: Partial<Stripe.SetupIntent> = {}): Stripe.SetupIntent {
  return {
...
export function createCreditNote(overrides: Partial<Stripe.CreditNote> = {}): Stripe.CreditNote {
  const lines = overrides.lines ?? {
    object: "list",
    has_more: false,
    total_count: 1,
